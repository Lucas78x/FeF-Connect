...
{
  public class FinDocumentController : Controller
  {
    ...
    private readonly ILogger<FinDocumentController> _logger;

    public FinDocumentController(IValidateSession validateSession, IConfiguration configuration,
                              IWebHostEnvironment webHostEnvironment, IFileEncryptor fileEncryptor,
                              IHttpClientFactory httpClientFactory, ILogger<FinDocumentController> logger)
    {
    ...
    public async Task<IActionResult> FinDocument()
    {
      ...
      try
      {
        ...
        if (response.IsSuccessStatusCode)
        {
          ...
          return View("FinDocument", partialModel);
        }
      }
      catch (Exception ex)
      {
      ...
      try
      {
        ...
        if (_validateSession.HasAdministrator(_validateSession.GetPermissao()))
          directoryPath = Path.Combine(_webHostEnvironment.WebRootPath,
                                         TipoPermissaoEnum.Financeiro.GetHashCode().ToString(),
                                         "Document"); ;
            ...
            if (parts.Length >= 3)
            {
              return new DocumentViewModel
              {
                ...
                FilePath = _validateSession.HasAdministrator(_validateSession.GetPermissao())
                      ? $"/{TipoPermissaoEnum.Financeiro.GetHashCode()}/Document/{Path.GetFileName(file)}"
