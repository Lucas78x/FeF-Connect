...
{
  public class FinTutorialController : Controller
  {
    private readonly IValidateSession _validateSession;
    private readonly IConfiguration _configuration;
    private readonly IWebHostEnvironment _webHostEnvironment;
    private readonly IFileEncryptor _fileEncryptor;
    public FinTutorialController(IValidateSession validateSession, IConfiguration configuration, IWebHostEnvironment webHostEnvironment, IFileEncryptor fileEncryptor)
    {
    ...
    public async Task<IActionResult> FinTutorial()
    {
      ...
      if(!_validateSession.HasFinanceiro(_validateSession.GetPermissao()) || !_validateSession.HasAdministrator(_validateSession.GetPermissao()))
      ...
      try
      {
        ...
        return View("FinTutorial", partialModel);
      }
      catch
      {
    ...
    public List<DocumentViewModel> GetDocumentsFromDirectory()
    {
      ...
      if (_validateSession.HasAdministrator(_validateSession.GetPermissao()))
        directoryPath = Path.Combine(Directory.GetCurrentDirectory(), $"wwwroot/{TipoPermissaoEnum.Financeiro.GetHashCode()}");
        ...
        if (parts.Length >= 3)
        {
          ...
          else
          {
            documents.Add(new DocumentViewModel
            {
              Title = title,
              Category = category,
              FilePath = $"/{TipoPermissaoEnum.Financeiro.GetHashCode()}/{Path.GetFileName(file)}",
              FileType = fileExtension,
              ImagePath = "/images/default.png"
            });
          }
    ...
    public async Task<IActionResult> Upload(IFormFile file, string title, string category)
    {
      if (file != null && file.Length > 0)
      {
        ...
        if(_validateSession.HasAdministrator(_validateSession.GetPermissao())) directoryPath = Path.Combine(Directory.GetCurrentDirectory(), $"wwwroot/{TipoPermissaoEnum.Financeiro.GetHashCode()}");
