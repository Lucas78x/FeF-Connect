...
public class FinanceiroController : Controller
{
  ...
  public async Task<IActionResult> RegistroT()
  {
    if (!IsUserValid()) return RedirectToLogin();

    if (!_validateSession.HasFinanceiro(_validateSession.GetPermissao())) return RedirectToErrorPage();
  ...
  private string GetUserDirectory() =>
      Path.Combine(Directory.GetCurrentDirectory(), $"wwwroot/Partial/{_validateSession.GetPermissao().GetHashCode()}/Relatorios");

  private void SaveFile(string folderName, string fileName, IFormFile fileUpload)
  {
    // Verifica se o arquivo de upload tem uma extensão
    ...
    // Garante que o arquivo salvo terá a mesma extensão que o arquivo de upload
